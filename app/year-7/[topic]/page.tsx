...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const meta = getTopicMeta(7, topic);
    return {
      title: `${meta.title} | Year 7 | Open Math`,
...
export default async function TopicPage({ params }: Props) {
  ...
  const subtopics = topicMeta.subtopics.map((id) => {
    try {
      return getSubtopicMeta(7, topic, id);
    } catch {
      return null;
    }
  }).filter(Boolean);

  const worksheetSections = getWorksheetSections(7, topic, topicMeta.subtopics);
