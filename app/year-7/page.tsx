...
export const metadata: Metadata = {
  title: "Year 7 | Open Math",
  description: "Year 7 NSW Mathematics practice topics — free, no account needed.",
};
...
function getTopicIds(): string[] {
  const base = path.join(process.cwd(), "content", "year-7");
  const dirs = fs
...
export default function Year7Page() {
  const topics = getTopicIds().map((id) => getTopicMeta(7, id));
    ...
    <div className="flex flex-col min-h-full">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <nav className="text-sm text-slate-400 mb-8">
          ...
          <span className="text-slate-600">Year 7</span>
        </nav>

        <h1 className="text-3xl font-bold text-slate-900 mb-2 tracking-tight">Year 7</h1>
