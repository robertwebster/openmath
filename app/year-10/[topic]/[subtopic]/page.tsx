...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const meta = getSubtopicMeta(10, topic, subtopic);
    return {
...
export default async function SubtopicPage({ params }: Props) {
  ...
  try {
    topicMeta = getTopicMeta(10, topic);
    subtopicMeta = getSubtopicMeta(10, topic, subtopic);
    questions = getQuestions(10, topic, subtopic);
    workedExamples = getWorkedExamples(10, topic, subtopic);
  } catch {
    notFound();
  }
    ...
    <div className="flex flex-col min-h-full">
      <JsonLd
        data={[
          breadcrumbJsonLd([
            { name: "Open Math", path: "/" },
            { name: "Year 10", path: "/year-10" },
            { name: topicMeta.title, path: `/year-10/${topic}` },
            { name: subtopicMeta.title },
          ]),
          learningResourceJsonLd({
            name: subtopicMeta.title,
            description: subtopicMeta.description,
            path: `/year-10/${topic}/${subtopic}`,
            year: 10,
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <nav className="text-sm text-slate-400 mb-8 print:hidden">
          ...
          <Link href="/year-10" className="hover:text-slate-600 transition-colors">Year 10</Link>
          <span className="mx-2">›</span>
          <Link href={`/year-10/${topic}`} className="hover:text-slate-600 transition-colors">{topicMeta.title}</Link>
          <span className="mx-2">›</span>
