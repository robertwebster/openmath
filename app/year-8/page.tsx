...
export const metadata: Metadata = {
  title: "Year 8 | Open Math",
  description: "Year 8 NSW Mathematics practice topics — free, no account needed.",
};
...
function getTopicIds(): string[] {
  const base = path.join(process.cwd(), "content", "year-8");
  const dirs = fs
...
export default function Year8Page() {
  const topics = getTopicIds().map((id) => getTopicMeta(8, id));
    ...
    <div className="flex flex-col min-h-full">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <nav className="text-sm text-slate-400 mb-8">
          ...
          <span className="text-slate-600">Year 8</span>
        </nav>

        <h1 className="text-3xl font-bold text-slate-900 mb-2 tracking-tight">Year 8</h1>
        <p className="text-slate-500 mb-10">Stage 4 · NSW Mathematics syllabus</p>

        <div className="grid gap-4 sm:grid-cols-2">
          {topics.map((topic) => (
            <Link
              key={topic.id}
              href={`/year-8/${topic.id}`}
