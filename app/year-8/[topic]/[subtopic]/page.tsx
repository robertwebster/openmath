...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const meta = getSubtopicMeta(8, topic, subtopic);
    return {
...
export default async function SubtopicPage({ params }: Props) {
  ...
  try {
    topicMeta = getTopicMeta(8, topic);
    subtopicMeta = getSubtopicMeta(8, topic, subtopic);
    questions = getQuestions(8, topic, subtopic);
    workedExamples = getWorkedExamples(8, topic, subtopic);
  } catch {
    notFound();
  }
    ...
    <div className="flex flex-col min-h-full">
      <JsonLd
        data={[
          breadcrumbJsonLd([
            { name: "Open Math", path: "/" },
            { name: "Year 8", path: "/year-8" },
            { name: topicMeta.title, path: `/year-8/${topic}` },
            { name: subtopicMeta.title },
          ]),
          learningResourceJsonLd({
            name: subtopicMeta.title,
            description: subtopicMeta.description,
            path: `/year-8/${topic}/${subtopic}`,
            year: 8,
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <nav className="text-sm text-slate-400 mb-8 print:hidden">
          ...
          <Link href="/year-8" className="hover:text-slate-600 transition-colors">Year 8</Link>
          <span className="mx-2">›</span>
          <Link href={`/year-8/${topic}`} className="hover:text-slate-600 transition-colors">{topicMeta.title}</Link>
          <span className="mx-2">›</span>
