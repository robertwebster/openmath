...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const meta = getTopicMeta(8, topic);
    return {
      title: `${meta.title} | Year 8 | Open Math`,
...
export default async function TopicPage({ params }: Props) {
  ...
  const subtopics = topicMeta.subtopics.map((id) => {
    try {
      return getSubtopicMeta(8, topic, id);
    } catch {
      return null;
    }
  }).filter(Boolean);

  const worksheetSections = getWorksheetSections(8, topic, topicMeta.subtopics);
